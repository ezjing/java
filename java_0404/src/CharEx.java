public class CharEx {
    public static void main(String[] args) {
        ...
        // 변수 c1 : char -> int로 데이터 변환한것(자동 타입 변환). 기본 타입은 서로 다른 데이터 타입으로 변환 가능

        // 자동(묵시적) 타입 변환 : 작은데이터타입 -> 큰데이터 타입 (정수형보다 실수형이 더 큰 데이터 타입(소수점까지 표현 되니까) : byte(1) < short(2) = char(2) < int(4) < long(8) < float(4) < double(8)) (int, double정도만 실습에서 사용)
        // 강제(명시적) 타입 변환 : 큰 데이터 타입 -> 작은 데이터 타입
