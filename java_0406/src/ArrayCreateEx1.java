public class ArrayCreateEx1 {
    public static void main(String[] args) {
        // 배열 : JS의 배열과 기본적인 특징은 비슷하지만 다른 점도 있음
        // 다른점
        // 1. 자바의 배열은 생성 시 지정된 크기를 변경할 수 없음 (크기란 데이터가 몇개 들어가 있는지를 뜻하는듯)
        // 2. 자바의 배열을 동일한 데이터 타입만 저장할 수 있음
        ...
        System.out.println("numArr1[4] : " + numArr1[4]);

        // 2. 배열 선언과 동시에 데이터를 저장하는 방식
        // 편의를 위해서 new 부분을 생략해도 됨
        // {} 안의 데이터의 개수에 따라서 배열의 크기가 결정됨 (타입뒤의 []에 굳이 크기지정을 할 필요가 없게됨)
        int[] numArr2 = {10, 20, 30, 40, 50};
        // int[] numArr2 = new int[] {10, 20, 30, 40, 50};  // 이런식으로 사용해도 됨 (2, 3 합한 방식)

        System.out.println("배열 numArr2[0] : " + numArr2[0]);
        System.out.println("배열 numArr2[4} : " + numArr2[4]);

        // 3. 배열 타입의 변수만 먼저 생성하고 필요에 따라서 배열 객체를 생성하여 사용하는 방식
        // 배열의 크기가 결정되지 않았기 떄문에 나중에 배열객체 생성 시 크기를 결정할 수 있도록 반드시 new 키워드를 사용해야 함
        // new 키워드 없이 { 데이터 } 형식으로 사용하는 것은 지원하지 않음
        int[] numArr3;
//        numArr3 = {10, 20, 30, 40, 50};
        numArr3 = new int[] {10, 20, 30, 40, 50};
        // numArr3 = new int[5];    // 사용가능한 방법, 밑에 배열을 채워주는 데이터만 써주면 됨
        ...
        // 기본값은 배열 안에 아무런 데이터가 없다는 뜻이란 의미로 해당 값들이 출력됨
        ...
        // numArr1.length = 10; // 이런식으로 값을 입력하는 것은 안됨, length는 읽기 전용임
